...
def get_time_embedding(time_steps, t_emb_dim):
    ...
    t_emb = torch.stack([torch.sin(t_emb), torch.cos(t_emb)], dim=-1)
    return t_emb
...
class DownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, t_emb_dim, down_sample, num_heads):
        ...
        self.residual_input_conv = nn.Conv2d(in_channels, out_channels, kernel_size=4, stride=2, padding = 1) if self.down_sample else nn.Identity()
    ...
    def forward(self, x , t_emb):
        ...
        out = out + self.t_emb_layes(t_emb)[:, :, None, None]
